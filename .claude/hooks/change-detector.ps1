...
function Create-ChangeRequest {
    ...
    if (Test-Path $templatePath) {
        ...
        # Replace tokens
        $changeRequest = $template -replace "BMAD-CC", $PROJECT_NAME `
        ...
        Write-Host "🔄 Change request created: $changeId" -ForegroundColor Yellow
        Write-Host "📝 Review at: $changeFile" -ForegroundColor Gray
...
function Trigger-ChangeValidation {
    ...
    Write-Host "🔍 Triggering change impact validation..." -ForegroundColor Blue
    
    try {
        & ./.claude/hooks/validation-enforcer.ps1 -EventType "change-request" -Context @{ChangeId=$ChangeId}
        Write-Host "✅ Change validation triggered successfully" -ForegroundColor Green
    } catch {
        Write-Warning "⚠️ Change validation failed: $_"
    }
}
...
switch ($EventType) {
    "file-modified" {
        if ($FilePath -and (Test-Path $FilePath)) {
            ...
            if (Detect-ScopeChange -FilePath $FilePath -Content $content) {
                ...
                Write-Host "📊 Scope change detected in: $FilePath" -ForegroundColor Yellow
            }
            ...
            if (Detect-ArchitectureChange -FilePath $FilePath -Content $content) {
                ...
                Write-Host "🏗️ Architecture change detected in: $FilePath" -ForegroundColor Yellow
            }
    ...
    "prd-updated" {
        Write-Host "📋 PRD update detected - checking for scope changes..." -ForegroundColor Blue
    ...
    "architecture-updated" {
        Write-Host "🏗️ Architecture update detected - checking for breaking changes..." -ForegroundColor Blue
    ...
    "check-pending-changes" {
        ...
        if (Test-Path ".bmad-change-pending") {
            ...
            Write-Host "⚠️ Pending change request: $pendingChangeId" -ForegroundColor Yellow
            ...
            if ($validationFile) {
                Write-Host "📋 Change validation exists: $($validationFile.Name)" -ForegroundColor Green
            } else {
                Write-Host "❌ Change validation missing - run change impact assessment" -ForegroundColor Red
            }
        } else {
            Write-Host "✅ No pending changes detected" -ForegroundColor Green
        }
    }
    ...
    "approve-change" {
        ...
        if (Test-Path ".bmad-change-pending") {
            ...
            if (Test-Path $changeFile) {
                ...
                Write-Host "✅ Change $pendingChangeId approved and archived" -ForegroundColor Green
            }
        } else {
            Write-Host "ℹ️ No pending changes to approve" -ForegroundColor Gray
        }
    }
...
foreach ($configFile in $configFiles) {
    if ($FilePath -eq $configFile -and (Test-Path $configFile)) {
        Write-Host "⚙️ Configuration change detected in: $configFile" -ForegroundColor Blue
