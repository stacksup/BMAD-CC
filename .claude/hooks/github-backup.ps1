    ...
    if (-not $remoteUrl) {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-Host "✅ Created GitHub repository: $ghUser/$RepoName" -ForegroundColor Green
                return $true
            } else {
...
function Backup-ToGitHub {
    ...
    Write-Host "`n🔄 GitHub Backup Process Starting..." -ForegroundColor Cyan
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "⚠️ Pull failed, attempting to push anyway..." -ForegroundColor Yellow
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "⚠️ Direct push failed. Trying with upstream..." -ForegroundColor Yellow
    ...
    if ($remoteUrl) {
        ...
        Write-Host "📎 View on GitHub: $webUrl" -ForegroundColor Cyan
    }
...
switch ($Action) {
    ...
    "status" {
        $connection = Test-GitHubConnection
        Write-Host "`n📊 GitHub Integration Status" -ForegroundColor Cyan
        Write-Host "=" * 40
        ...
        if ($connection.Connected) {
            ...
            Write-Host "❌ GitHub Not Connected: $($connection.Message)" -ForegroundColor Red
    ...
    "push-all" {
        ...
        Write-Host "📤 Pushing all branches to GitHub..." -ForegroundColor Cyan
        git push --all origin
