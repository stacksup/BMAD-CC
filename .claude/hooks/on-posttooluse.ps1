#!/usr/bin/env pwsh
# Post-tool-use hook for BMAD-CC
...
if ($gitStatus) {
    ...
    foreach ($file in $changedFiles) {
        ...
        if ($file -match "api|routes|endpoints|controllers") {
            ...
            Write-Host "📝 API changes detected - remember to update documentation" -ForegroundColor Yellow
        }
        if ($file -match "package\.json|requirements\.txt|go\.mod|pom\.xml") {
            $needsDocumentation = $true
            Write-Host "📝 Dependency changes detected - update README prerequisites" -ForegroundColor Yellow
        }
        if ($file -match "docker|Dockerfile|compose") {
            $needsDocumentation = $true
            Write-Host "📝 Docker changes detected - update deployment docs" -ForegroundColor Yellow
        }
        if ($file -match "\.env|config|settings") {
            $needsDocumentation = $true
            Write-Host "📝 Configuration changes detected - update config docs" -ForegroundColor Yellow
        }
    }
    
    # Remind about documentation if needed
    if ($needsDocumentation) {
        Write-Host "`n💡 Documentation Reminder:" -ForegroundColor Cyan
...
$currentTask = $null
try {
    $currentTask = & task-master current --json 2>$null | ConvertFrom-Json
} catch {}

if ($currentTask -and $significantChanges) {
    Write-Host "`n📋 Current Task: [$($currentTask.id)] $($currentTask.title)" -ForegroundColor Cyan
    Write-Host "   Remember to update task status when complete" -ForegroundColor Gray
}

# Check documentation health periodically
$lastDocCheck = Join-Path $env:TEMP "last-doc-check-BMAD-CC.txt"
...
if ($shouldCheckDocs) {
    ...
    if ($docIssues.Count -gt 0) {
        Write-Host "`n⚠️  Documentation Health Check:" -ForegroundColor Yellow
