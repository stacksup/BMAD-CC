...
if command -v task-master &> /dev/null; then
    current_task=$(task-master current --json 2>/dev/null || true)
elif command -v npx &> /dev/null; then
    current_task=$(npx task-master current --json 2>/dev/null || true)
fi
...
last_doc_check="/tmp/last-doc-check-BMAD-CC.txt"
