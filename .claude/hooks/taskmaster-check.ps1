...
function Initialize-TaskMaster {
    Write-Host "🚀 Initializing Task Master for $PROJECT_NAME..." -ForegroundColor Cyan
    ...
    Write-Host "✅ Task Master initialized successfully" -ForegroundColor Green
}
...
switch ($Command) {
    "check" {
        ...
        if (-not (Test-TaskMasterAvailable)) {
            Write-Error "❌ TASK MASTER NOT AVAILABLE!"
        ...
        if (-not (Test-TaskMasterProject)) {
            Write-Warning "⚠️ Task Master not initialized in this project"
            Write-Host "Initializing now..." -ForegroundColor Yellow
            Initialize-TaskMaster
        }
        
        Write-Host "✅ Task Master is available and configured" -ForegroundColor Green
        
        # Show current task status
        $current = Get-CurrentTask
        if ($current) {
            Write-Host "`n📋 Current Task: $($current.title)" -ForegroundColor Cyan
            Write-Host "   ID: $($current.id) | Status: $($current.status) | Priority: $($current.priority)"
        } else {
            Write-Host "`n📋 No tasks currently assigned" -ForegroundColor Gray
        }
    }
    ...
    "create" {
        ...
        if ($title) {
            ...
            Write-Host "✅ Created task: $($task.id) - $($task.title)" -ForegroundColor Green
        }
    }
