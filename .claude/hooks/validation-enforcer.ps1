...
# Configurable minimum scores by validation type - PowerShell 5.1 compatible
$archScore = if ($env:BMAD_ARCH_MIN_SCORE) { $env:BMAD_ARCH_MIN_SCORE } else { 8 }
$prdScore = if ($env:BMAD_PRD_MIN_SCORE) { $env:BMAD_PRD_MIN_SCORE } else { 8 }
$setupScore = if ($env:BMAD_SETUP_MIN_SCORE) { $env:BMAD_SETUP_MIN_SCORE } else { 7 }
$storyScore = if ($env:BMAD_STORY_MIN_SCORE) { $env:BMAD_STORY_MIN_SCORE } else { 7 }
$dodScore = if ($env:BMAD_DOD_MIN_SCORE) { $env:BMAD_DOD_MIN_SCORE } else { 9 }
$changeScore = if ($env:BMAD_CHANGE_MIN_SCORE) { $env:BMAD_CHANGE_MIN_SCORE } else { 7 }

$MIN_SCORES = @{
    "architecture" = [int]$archScore
    "prd" = [int]$prdScore
    "project-setup" = [int]$setupScore
    "story-draft" = [int]$storyScore
    "story-dod" = [int]$dodScore
    "change-impact" = [int]$changeScore
}
